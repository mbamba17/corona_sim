...
ggplot(subset(pom,datum<"2020-04-30"),aes(x=datum,y=values,col=regija)) + geom_line(size=2) + geom_point(data=subset(pom,datum>"2020-04-30"),size=4)+ theme(legend.position = "top",legend.title = element_blank()) + labs(x="",y="",title="Nezabilježena kontrakcija pouzdanja u uslužnom sektoru",subtitle = "Indeks pouzdanja uslužnog sektora",caption ="Izvor: Eurostat") + scale_x_date(breaks = "1 year",labels = scales::date_format("%y")) + boje_col
...
pom1 <- get_eurostat(id="namq_10_a10") %>% filter(s_adj=="SCA" & na_item=="B1G" & unit=="CP_MNAC" & geo %in% c("AT","DE","ES","FR","LT","NL","SK","UK","NO")) %>% mutate(datum = ceiling_date(time,"quarter")-1) %>% select(datum,geo,sektor=nace_r2,values) %>% arrange(datum) %>% group_by(geo,sektor)%>% mutate(dbdv=round((values/lag(values,4)-1)*100,1)) %>% filter(datum=="2020-03-31") %>% select(-datum,-values) %>% spread(geo,dbdv)
pom2 <- read_excel(path = "radni.xlsx",sheet = "bdp",range = "BM4:BQ16")
pom <- left_join(pom1,pom2,by="sektor") %>% gather(key = "geo",value = "dbdv",-sektor) %>% mutate(regija=case_when(!geo %in% c("S1","S2","S3","HR")~"bla",T~geo)) %>% ungroup() %>% mutate(sektor=case_when(sektor=="A"~"Poljoprivreda",sektor=="B-E"~"Industrija",sektor=="C"~"Prerađivačka industrija",sektor=="F"~"Građevinarstvo",sektor=="G-I"~"Trgovina, prijevoz i smještaj",sektor=="J"~"Informacije",sektor=="K"~"Financije",sektor=="L"~"Promet nekretninama",sektor=="M_N"~"Znanost i administracija",sektor=="O-Q"~"Javna uprava",sektor=="R-U"~"Ostale usluge",sektor=="TOTAL"~"Ukupno")) %>% filter(!geo %in% c("S1","S2","S3"))
...
ggplot(pom1,aes(x=datum,y=iznos)) + geom_line(size=2) + geom_line(data = pom2,size=2,aes(x=datum,y=iznos,col=scenarij)) + labs(x="",y="",title="Nominalne bruto plaće",subtitle="Desezonirana vrijednost, administrativni izvori",caption="Izvor: Hanfa") + boje_col + geom_text(data=subset(pom2, datum == "2020-12-31"),aes(label=izns,col=scenarij),hjust=0, vjust=0, show.legend = FALSE,position=position_jitter(height=500)) +  scale_x_date(breaks = "1 year",labels = date_format("%y"),limits = c(as.Date("2001-01-01"),as.Date("2021-06-01")))
...
ggplot(pom1,aes(x=datum,y=iznos)) + geom_line(size=2) + geom_line(data = pom2,size=2,aes(x=datum,y=iznos,col=scenarij)) + labs(x="",y="",title="Tečaj EUR/HRK",subtitle="",caption="Izvor: Hanfa") + boje_col + geom_text(data=subset(pom2, datum == "2020-12-31"),aes(label=izns,col=scenarij),hjust=-0.1, vjust=0, show.legend = FALSE,position=position_jitter(height=0.1)) 
...
ggplot(pom1,aes(x=datum,y=iznos)) + geom_line(size=2) + geom_line(data = pom2,size=2,aes(x=datum,y=iznos,col=scenarij)) + labs(x="",y="",title="Inflacija",subtitle="godišnja stopa rasta (%)",caption="Izvor: Hanfa") + boje_col + geom_text(data=subset(pom2, datum == "2020-12-31"),aes(label=izns,col=scenarij),hjust=-0.01, vjust=0, show.legend = FALSE,position=position_jitter(height=1)) 
...
ggplot(pom1,aes(x=datum,y=iznos)) + geom_line(size=2) + geom_line(data = pom2,size=2,aes(x=datum,y=iznos,col=scenarij)) + labs(x="",y="",title="Kamatne stope na kredite realnom sektoru",subtitle="Kamatna stopa na stanja dugoročnih kredita banaka sektorima kućanstava i nefinancijskih poduzeća (%)",caption="Izvor: Hanfa") + boje_col + geom_text(data=subset(pom2, datum == "2020-12-31"),aes(label=izns,col=scenarij),hjust=0, vjust=0, show.legend = FALSE,position=position_jitter(height=0.5)) +  scale_x_date(breaks = "1 year",labels = date_format("%y"),limits = c(as.Date("2006-01-01"),as.Date("2021-01-01")))
...
# 21. Ostvarenje q1 2020 u povijesnoj distribuciji ####

pom1 <- get_eurostat(id="namq_10_a10") %>% filter(s_adj=="SCA" & na_item=="B1G" & unit=="CP_MNAC" & geo %in% c("HR")) %>% mutate(datum = ceiling_date(time,"quarter")-1) %>% select(datum,geo,sektor=nace_r2,values) %>% arrange(datum) %>% group_by(geo,sektor) %>% mutate(dbdv=(values/lag(values,4)-1)*100) %>% select(-values) %>% ungroup() %>% mutate(sektor=case_when(sektor=="A"~"Poljoprivreda",sektor=="B-E"~"Industrija",sektor=="C"~"Prerađivačka industrija",sektor=="F"~"Građevinarstvo",sektor=="G-I"~"Trgovina, prijevoz i smještaj",sektor=="J"~"Informacije",sektor=="K"~"Financije",sektor=="L"~"Promet nekretninama",sektor=="M_N"~"Znanost i administracija",sektor=="O-Q"~"Javna uprava",sektor=="R-U"~"Ostale usluge",sektor=="TOTAL"~"Ukupno")) %>% filter(datum>"2008-12-31")
pom3 <- pom1 %>% group_by(sektor) %>% summarise(medijan=median(dbdv,na.rm = T),pc25=quantile(dbdv,probs = 0.25,na.rm=T),pc75=quantile(dbdv,probs = 0.75,na.rm=T))
pom2 <- read_excel(path = "radni.xlsx",sheet = "bdp",range = "BM4:BQ16") %>% select(sektor,HR)%>% mutate(sektor=case_when(sektor=="A"~"Poljoprivreda",sektor=="B-E"~"Industrija",sektor=="C"~"Prerađivačka industrija",sektor=="F"~"Građevinarstvo",sektor=="G-I"~"Trgovina, prijevoz i smještaj",sektor=="J"~"Informacije",sektor=="K"~"Financije",sektor=="L"~"Promet nekretninama",sektor=="M_N"~"Znanost i administracija",sektor=="O-Q"~"Javna uprava",sektor=="R-U"~"Ostale usluge",sektor=="TOTAL"~"Ukupno"))
ggplot(pom1,aes(x=dbdv)) + geom_density() + geom_rect(data = pom3,aes(ymin=-Inf,ymax=+Inf,xmin=pc25,xmax=pc75),inherit.aes = FALSE,alpha=0.3,fill="#155e63") + geom_vline(data = pom2,aes(xintercept=HR),col="#e84545",size=1.3) + geom_vline(data = pom3,aes(xintercept=medijan),col="#155e63",size=1.3) + facet_wrap(~sektor,scales="free") + labs(x="",x="")
