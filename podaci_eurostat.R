...
ggplot(pom,aes(x=datum,y=doprinos)) + geom_col(alpha=0.8,aes(fill=sektor))  + geom_line(data = pom2,aes(x=datum,y=dbdv),size=1.5) + facet_wrap(~regija) + boje_fill + labs(x="",y="Doprinosi realnom rastu BDV-a (u %)",title = "Najveći dio rasta BDV-a generira trgovina i uslužne djelatnosti",subtitle = "Doprinosi pojedinih djelatnosti realnom rastu bruto dodane vrijednosti",caption = "Izvor: Eurostat") + scale_y_continuous(breaks = -7:7)
# doprinosi rastu za AT, ES i FR koji imaju podatke za Q1
pom <- pom <- get_eurostat(id="namq_10_a10") %>% filter(s_adj=="SCA" & na_item=="B1G" & unit=="CP_MNAC" & geo %in% c("AT","ES","FR")) %>% mutate(datum = ceiling_date(time,"quarter")-1) %>% select(datum,geo,sektor=nace_r2,values) %>% arrange(datum) %>% group_by(geo,sektor)%>% mutate(dbdv=round((values/lag(values,4)-1)*100,1)) %>% filter(datum>="2017-12-31")
ggplot(pom,aes(x=datum,y=dbdv)) + geom_line(size=1.5) + facet_grid(sektor~geo,scales = "free") + scale_x_date(breaks = "1 year",labels = scales::date_format("%y"),limits = c(as.Date("2017-12-31"),as.Date("2020-06-30"))) + geom_text(data = subset(pom, datum == "2020-03-31"), aes(label = dbdv, x = datum, y = dbdv,hjust=-0.2,vjust=-0.1)) + geom_hline(yintercept = 0,col="red",linetype=2)
pom1 <- pom %>% filter(datum=="2020-03-31") %>% select(sektor,geo,dbdv) %>% spread(geo,dbdv)
