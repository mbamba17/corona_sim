...
temp <- read_excel("dionice_zadnji_datum.xlsx",sheet = "vrijednosti_mjesecni",skip = 5,na = c("#N/A N/A","#N/A Invalid Security")) %>% gather(key = "isin",value = "cijena",-datum) %>% mutate(datum=as.Date(datum)) %>% na.omit() %>% arrange(datum) %>% group_by(isin) %>% mutate(povrat=cijena/lag(cijena,6)-1) %>% group_by(isin) %>% summarise(var=quantile(povrat,kvantil_dionice,na.rm=T))
# spajanje i izračun
...
# Izračun value at riska - na podacima s bloomberga
temp <- read_excel("ifondovi_zadnji_datum.xlsx",sheet = "vrijednosti_mjesecni",skip = 5,na = c("#N/A N/A","#N/A Invalid Security")) %>% gather(key = "isin",value = "cijena",-datum) %>% mutate(datum=as.Date(datum)) %>% na.omit() %>% arrange(datum) %>% group_by(isin) %>% mutate(povrat=cijena/lag(cijena,6)-1) %>% group_by(isin) %>% summarise(var=quantile(povrat,kvantil_ifondovi,na.rm=T))
# spajanje i izračun
...
pom1 <- imovina %>% filter(vrsta0=="Mirovinski" & datum=="2020-12-31") %>% group_by(vrsta1) %>% summarise(iznos=sum(iznos,na.rm=T)) %>% left_join(data.frame(vrsta1=c("Dobrovoljan","Obavezan"),doprinos=c(doprinosi_dmf,doprinosi_omf)),by="vrsta1") %>% mutate(mf_faktor=1+doprinos/iznos) %>% select(vrsta1,mf_faktor)
imovina <- imovina %>% left_join(pom1,by="vrsta1") %>% mutate(iznos=ifelse(vrsta0=="Mirovinski" & datum=="2020-12-31",iznos*mf_faktor,iznos)) %>% select(-mf_faktor)
rm(pom1)
